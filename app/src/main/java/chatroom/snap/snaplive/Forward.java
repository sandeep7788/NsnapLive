...
import chatroom.snap.snaplive.adapters.ForwardAdapter;
import chatroom.snap.snaplive.adapters.ForwardAdapterG;
                                ...
                                if (name.equals(phone))
                                    for (UserData mContactIterator : contactList) {
                                        if (mContactIterator.getPhone().equals(mUser.getPhone())) {
                                            mUser.setName(getContactName(mUser.getPhone(), Forward.this));
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if (mAuth.getCurrentUser() != null) {
                        ...
                        chatroom.snap.snaplive.Notification.Message message1 = new chatroom.snap.snaplive.Notification.Message(Global.nameLocal, Global.avaLocal,
                                friendid, message, null, mAuth.getCurrentUser().getUid(),
                                Mid);
                    ...
                    mTokenget.child(Global.groupsArray.get(halbineG(Global.groupsArray, friendid)).getUsers().get(i)).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        //Message maybe not encrypted
                            ...
                            chatroom.snap.snaplive.Notification.Message message2 = new chatroom.snap.snaplive.Notification.Message(Global.currname,Global.currAva,
                                    Global.groupsArray.get(halbineG(Global.groupsArray, friendid)).getUsers().get(finalI)  ,message,null,friendid,Mid       );
                            FirebaseDatabase.getInstance().getReference(Global.USERS).child(friendid).child(Global.device).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    if (dataSnapshot.exists()){
